...
<div class="max-w-3xl">
  <div class="bg-white rounded-3xl shadow-soft border border-gray-100 overflow-hidden">
    ...
    <div class="px-6 md:px-8 py-6 border-b border-gray-100 bg-gradient-to-br from-white to-gray-50">
      <div class="flex items-center justify-between gap-4">
        <div>
          ...
          <p class="text-sm text-gray-500 mt-1">Update category details and status.</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.categories.update', $category) }}" enctype="multipart/form-data" class="px-6 md:px-8 py-6">
      @csrf
      @method('PUT')
      ...
      <div class="mt-8 flex gap-3">
        <button
          class="flex-1 rounded-2xl bg-red-900 text-white py-3.5 text-sm font-semibold shadow-lg shadow-red-900/20 hover:bg-red-950 transition">
          Save Changes
        </button>
        <a href="{{ route('admin.categories.index') }}"
