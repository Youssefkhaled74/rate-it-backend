  ...
  <div class="bg-white rounded-3xl shadow-soft border border-gray-100 overflow-hidden">
    ...
    <div class="px-6 md:px-8 py-6 border-b border-gray-100 bg-gradient-to-br from-white to-gray-50">
      <div class="flex items-center justify-between gap-4">
        <div>
          <h2 class="text-xl font-semibold text-gray-900">Add New Brand</h2>
          <p class="text-sm text-gray-500 mt-1">Create a brand with logo and cover image.</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.brands.store') }}" enctype="multipart/form-data" class="px-6 md:px-8 py-6">
      @csrf
      @include('admin.brands._form', ['brand' => null])

      <div class="mt-8">
        <button class="w-full rounded-2xl bg-red-900 text-white py-3.5 text-sm font-semibold shadow-lg shadow-red-900/20 hover:bg-red-950 transition">
          Save
        </button>
      </div>
    </form>
