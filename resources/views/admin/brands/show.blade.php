@extends('admin.layouts.app')

@section('title','Brand Details')

@section('content')
@php
  $cover = $brand->cover_image ? asset($brand->cover_image) : ($brand->logo ? asset($brand->logo) : asset('assets/images/category-placeholder.png'));
  $logo = $brand->logo ? asset($brand->logo) : asset('assets/images/category-icon-placeholder.png');
@endphp

<div class="max-w-4xl space-y-6">
  <div class="bg-white rounded-3xl shadow-soft border border-gray-100 overflow-hidden">
    <div class="relative h-48 bg-gray-100">
      <img src="{{ $cover }}" alt="{{ $brand->name_en }}" class="w-full h-full object-cover">
      <div class="absolute top-4 right-4">
        <a href="{{ route('admin.brands.edit', $brand) }}"
           class="rounded-2xl bg-white/90 border border-gray-100 px-4 py-2.5 text-sm font-semibold text-gray-700 hover:bg-white transition">
          Edit
        </a>
      </div>
    </div>

    <div class="p-6">
      <div class="flex items-center gap-4">
        <div class="w-16 h-16 rounded-full bg-gray-50 overflow-hidden border border-gray-100 grid place-items-center">
          <img src="{{ $logo }}" class="w-16 h-16 object-cover" alt="logo">
        </div>
        <div>
          <div class="text-xl font-semibold text-gray-900">{{ $brand->name_en }}</div>
          <div class="text-sm text-gray-500">{{ $brand->name_ar ?: '-' }}</div>
        </div>
      </div>

      <div class="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="rounded-2xl bg-gray-50 border border-gray-100 p-4">
          <div class="text-xs text-gray-500">Status</div>
          <div class="text-sm font-semibold text-gray-900 mt-1">
            {{ $brand->is_active ? 'Active' : 'Inactive' }}
          </div>
        </div>
        <div class="rounded-2xl bg-gray-50 border border-gray-100 p-4">
          <div class="text-xs text-gray-500">Places</div>
          <div class="text-sm font-semibold text-gray-900 mt-1">{{ $brand->places_count ?? 0 }}</div>
        </div>
        <div class="rounded-2xl bg-gray-50 border border-gray-100 p-4">
          <div class="text-xs text-gray-500">Created</div>
          <div class="text-sm font-semibold text-gray-900 mt-1">{{ $brand->created_at?->format('Y-m-d') }}</div>
        </div>
      </div>

      <div class="mt-6">
        <div class="text-sm font-semibold text-gray-800">Description (EN)</div>
        <div class="text-sm text-gray-600 mt-2">{{ $brand->description_en ?: '-' }}</div>
      </div>

      <div class="mt-4">
        <div class="text-sm font-semibold text-gray-800">Description (AR)</div>
        <div class="text-sm text-gray-600 mt-2">{{ $brand->description_ar ?: '-' }}</div>
      </div>
    </div>
  </div>

  <div>
    <a href="{{ route('admin.brands.index') }}"
       class="rounded-2xl bg-white border border-gray-200 px-5 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition">
      Back to Brands
    </a>
  </div>
</div>
@endsection
