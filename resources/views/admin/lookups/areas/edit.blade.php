...
@section('title', __('admin.edit_area'))
  ...
  <div class="bg-white rounded-3xl shadow-soft border border-gray-100 overflow-hidden">
    ...
    <div class="px-6 md:px-8 py-6 border-b border-gray-100 bg-gradient-to-br from-white to-gray-50">
      <div class="flex items-center justify-between gap-4">
        <div>
          <h2 class="text-xl font-semibold text-gray-900">{{ __('admin.edit_area') }}</h2>
          <p class="text-sm text-gray-500 mt-1">{{ __('admin.manage_areas') }}</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.lookups.areas.update', $area) }}" class="px-6 md:px-8 py-6">
      @csrf
      @method('PUT')
      @include('admin.lookups.areas._form', ['area' => $area])
