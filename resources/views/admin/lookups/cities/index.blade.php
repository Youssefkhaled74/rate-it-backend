...
<div class="space-y-6">
  ...
  <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
    <h2 class="text-2xl font-semibold text-gray-900">{{ __('admin.cities') }}</h2>
  ...
  <div class="flex items-center gap-3">
    ...
    <a href="{{ route('admin.lookups.cities.index', ['status' => 'active', 'lang' => request('lang')]) }}"
    ...
    <a href="{{ route('admin.lookups.cities.index', ['status' => 'inactive', 'lang' => request('lang')]) }}"
            ...
            <tr class="hover:bg-gray-50/60 transition">
              ...
              <td class="py-4 px-5 text-right">
                <div class="inline-flex items-center gap-2">
                  <a href="{{ route('admin.lookups.cities.edit', $item) }}"
                     class="px-4 py-2 rounded-full bg-gray-100 text-gray-800 text-xs font-semibold hover:bg-gray-200 transition">
                    {{ __('admin.edit') }}
                  </a>
                  <form method="POST" action="{{ route('admin.lookups.cities.destroy', $item) }}">
                    @csrf
                    @method('DELETE')
                    <button type="button" data-confirm="delete-city-{{ $item->id }}" data-confirm-text="{{ __('admin.delete') }}" data-title="{{ __('admin.delete') }}" data-message="{{ __('admin.confirm_message') }}"
                      class="px-4 py-2 rounded-full bg-red-50 text-red-700 text-xs font-semibold hover:bg-red-100 transition">
                      {{ __('admin.delete') }}
                    </button>
                    <input type="hidden" name="_confirm_target" value="delete-city-{{ $item->id }}" />
                  </form>
                </div>
              </td>
            </tr>
