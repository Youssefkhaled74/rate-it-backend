...
@section('page_title', __('admin.edit_plan'))
@section('title', __('admin.edit_plan'))

@section('content')
  <div class="bg-white border border-gray-100 rounded-[24px] p-6 shadow-soft">
    <div class="flex items-center justify-between">
      <h2 class="text-2xl font-semibold text-gray-900">{{ __('admin.edit_plan') }}</h2>
      <a href="{{ route('admin.subscription-plans.index') }}" class="text-sm text-gray-500">{{ __('admin.back') }}</a>
    </div>
    ...
    <form method="POST" action="{{ route('admin.subscription-plans.update', $plan) }}" class="mt-6">
      @csrf
      @method('PATCH')
