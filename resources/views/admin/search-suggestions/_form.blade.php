@php
  $isEdit = !empty($suggestion);
@endphp

<div class="space-y-6">
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div>
      <label class="text-sm font-medium text-gray-700">{{ __('admin.term_en') }}</label>
      <input
        name="term_en"
        value="{{ old('term_en', $suggestion->term_en ?? '') }}"
    ...
    <div>
      <label class="text-sm font-medium text-gray-700">{{ __('admin.term_ar') }}</label>
      <input
        name="term_ar"
        value="{{ old('term_ar', $suggestion->term_ar ?? '') }}"
        class="mt-2 w-full rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm outline-none transition
               focus:border-red-300 focus:ring-4 focus:ring-red-100"
      >
    </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
    <div>
      ...
      <input
        name="sort_order"
        type="number"
        value="{{ old('sort_order', $suggestion->sort_order ?? 0) }}"
        class="mt-2 w-full rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm outline-none transition
               focus:border-red-300 focus:ring-4 focus:ring-red-100"
      >
    </div>
    <div class="flex items-end">
      <label class="inline-flex items-center gap-3 cursor-pointer select-none">
        <input type="checkbox" name="is_active" value="1"
               {{ old('is_active', $suggestion->is_active ?? 1) ? 'checked' : '' }}
               class="sr-only peer">
