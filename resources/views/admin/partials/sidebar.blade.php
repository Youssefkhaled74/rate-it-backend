@php
  ...
  $isRtl = app()->getLocale() === 'ar' || $currentLang === 'ar';
  $items = [
    ...
    ['label' => __('admin.vouchers'), 'route' => 'admin.vouchers.index', 'icon' => 'ticket'],
    ['label' => __('admin.rating_questions'), 'route' => 'admin.rating-questions.index', 'icon' => 'checklist'],
    ['label' => __('admin.featured_places'), 'route' => 'admin.featured-places.index', 'icon' => 'star'],
    ['label' => __('admin.search_suggestions'), 'route' => 'admin.search-suggestions.index', 'icon' => 'search'],
    ['label' => __('admin.kpi_reports'), 'route' => 'admin.kpi-reports.index', 'icon' => 'chart'],
    ['label' => __('admin.qr_management'), 'route' => 'admin.qr-management.index', 'icon' => 'qr'],
    ['label' => __('admin.subscription_plans'), 'route' => 'admin.subscription-plans.index', 'icon' => 'trophy'],
    ['label' => __('admin.subscriptions'), 'route' => 'admin.subscriptions.index', 'icon' => 'users'],
  ...
  $iconSvg = function ($name) {
    return match ($name) {
      ...
      'chart' => '<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M3 3v18h18"/><path d="M7 15v-4"/><path d="M12 15V7"/><path d="M17 15v-9"/></svg>',
      'qr' => '<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M4 4h6v6H4z"/><path d="M14 4h6v6h-6z"/><path d="M4 14h6v6H4z"/><path d="M14 14h2v2h-2z"/><path d="M18 14h2v6h-6v-2h4z"/></svg>',
      'ticket' => '<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M3 9V7a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v2a2 2 0 0 0 0 4v2a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-2a2 2 0 0 0 0-4z"/><path d="M8 7v10"/><path d="M16 7v10"/></svg>',
...
<aside class="admin-sidebar-spacer w-72 p-4">
  <div class="admin-sidebar-fixed admin-sidebar admin-sidebar-inner h-[calc(100vh-2rem)] fixed top-5 left-5 bottom-5 text-white flex flex-col">
    ...
    {{-- Logo --}}
    <div class="flex items-center admin-logo-row mb-8 {{ $isRtl ? 'text-right justify-end' : '' }}">
      <div class="w-12 h-12 rounded-2xl bg-red-800 border border-white/10 grid place-items-center overflow-hidden shadow-lg shadow-black/10 {{ $isRtl ? 'order-2' : '' }}">
        <img src="{{ asset('assets/images/Vector.png') }}" alt="Rateit" class="w-9 h-9 object-contain">
      </div>
      <div class="{{ $isRtl ? 'order-1' : '' }}">
      ...
      <nav class="space-y-1">
        @foreach($items as $it)
          ...
          <a href="{{ Route::has($it['route']) ? route($it['route'], ['lang' => $currentLang]) : '#' }}"
             class="admin-nav-item group flex items-center gap-3 px-4 py-3 transition {{ $isRtl ? 'text-right justify-end' : '' }}
                    {{ $active ? 'is-active' : '' }}">
            <span class="admin-nav-icon text-white/90 {{ $isRtl ? 'order-2' : '' }}">{!! $iconSvg($it['icon']) !!}</span>
            <span class="admin-nav-text text-sm font-medium {{ $isRtl ? 'order-1' : '' }}">{{ $it['label'] }}</span>
          </a>
        @endforeach
      </nav>
    </div>
    ...
    <div class="mt-auto pt-6">
      <form method="POST" action="{{ route('admin.logout') }}">
        @csrf
        <button type="submit"
          class="admin-logout w-full flex items-center gap-3 px-4 py-3 rounded-2xl hover:bg-white/10 transition {{ $isRtl ? 'text-right justify-end' : 'text-left' }}">
          <span class="text-white/90 {{ $isRtl ? 'order-2' : '' }}">
            {{-- logout icon --}}
          ...
          <span class="text-sm font-medium {{ $isRtl ? 'order-1' : '' }}">{{ __('admin.logout') }}</span>
        </button>
      </form>
    </div>
